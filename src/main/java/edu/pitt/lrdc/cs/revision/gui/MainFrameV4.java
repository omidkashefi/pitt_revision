...
public class MainFrameV4 extends JFrame {
	...
	RevisionDocument rd;
	private AdvBaseLevelPanelV4 panel;
	...
	public MainFrameV4() {
		...
		item.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				int returnVal = fc.showOpenDialog(MainFrameV4.this);

				if (returnVal == JFileChooser.APPROVE_OPTION) {
					File file = fc.getSelectedFile();
					try {
						MainFrameV4.this.load(file.getAbsolutePath());
		...
		item2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					...
					try {
						panel.registerRevision();
						boolean isSuccess = MainFrameV4.this.export(file
								.getAbsolutePath());
						if (isSuccess) {
							JOptionPane.showMessageDialog(MainFrameV4.this,
									"File saved.");
		...
		menuDemoItem.addActionListener(new ActionListener() {
	...
	public void load(String path) throws Exception {
		...
		/*
		 * if (splitPane != null) this.remove(splitPane); splitPane = new
		 * JSplitPane(JSplitPane.HORIZONTAL_SPLIT, panel, displayPanel); //
		 * splitPane.setDividerLocation(900); // splitPane.setResizeWeight(0.5);
		 * splitPane.setDividerLocation(0.7); this.add(splitPane);
		 */
		this.add(panel);
	...
	public boolean save() {
		try {
			...
			if (reviewD != null)
				ReviewProcessor.writeReviewDocument(reviewD,
						reviewD.getDocName());
		} catch (Exception exp) {
			exp.printStackTrace();
			return false;
	...
	public static void main(String[] args) throws Exception {
		...
		MainFrameV4 mf = new MainFrameV4();
		// mf.load("dummy");
		mf.show();
