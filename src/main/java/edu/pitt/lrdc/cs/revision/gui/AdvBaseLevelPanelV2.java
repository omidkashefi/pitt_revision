...
public class AdvBaseLevelPanelV2 extends JPanel implements LevelPanel {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			if (lsm.isSelectionEmpty()) {
				...
				if (rus == null || currentRU.size() == 0) {
					...
					if(newList==null||newList.size()==0) newRU.setRevision_op(RevisionOp.DELETE);
					else if(oldList == null||oldList.size()==0) newRU.setRevision_op(RevisionOp.ADD);
					else newRU.setRevision_op(RevisionOp.MODIFY);
					currentRU.add(newRU);
				}
	...
	public AdvBaseLevelPanelV2(RevisionDocument doc) {
