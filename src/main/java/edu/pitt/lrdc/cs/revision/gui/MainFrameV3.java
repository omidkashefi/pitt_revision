...
public class MainFrameV3 extends JFrame {
	...
	RevisionDocument rd;
	private AdvBaseLevelPanelV3 panel;
		...
		item2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					...
					try {
						panel.registerRevision();
						boolean isSuccess = MainFrameV3.this.export(file
								.getAbsolutePath());
						if (isSuccess) {
							JOptionPane.showMessageDialog(MainFrameV3.this,
									"File saved.");
	...
	public void loadFromService(String serverAddress, String username)
		...
		MainFrameV3.this.load(ff.getAbsolutePath());
	...
	public void load(String path) throws Exception {
		 ...
		 */
		if (panel == null) {
			panel = new AdvBaseLevelPanelV3(rd);
			// this.add(panel);
		} else {
			this.remove(panel);
			panel = new AdvBaseLevelPanelV3(rd);
	...
	public static void main(String[] args) throws Exception {
		...
		MainFrameV3 mf = new MainFrameV3();
		// mf.load("dummy");
		mf.show();
